...
char maxchar(int n,char *clist)
{
    ...
    for (i = 0; i < n; i++)  count[clist[i] - 97]++;
    for (i = 0; i < 26; i++)  if (count[i] > max)  {max = count[i]; maxchar = i + 97;}
    return maxchar;
}
...
bool are_all_elements_equal(int n,double *v)
{
    ...
    bool result = true;
    #pragma omp parallel for
    for (i = 1; i < n; i++)  if (v[i] != v[0])  result = false;
    return result;
}
...
#pragma omp parallel num_threads(NTHREADS)
    {
        ...
        unsigned short right_fork = id;
        unsigned short left_fork = (id + 1) % NTHREADS;

        // Ensure that the lower-numbered fork is always locked first.
        if (id < (id + 1) % NTHREADS) {
